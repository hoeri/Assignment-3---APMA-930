...
% Compute the steady state solution to be able to compare the computed
% results to it

% Set the parabolic profile. 
 Vsteady = zeros(nx,ny-1) ;

 Usteady = zeros(nx-1,ny); 
 for n = 1:nx-1 
     Usteady(n,:) = y(1:end-1).*(ly - y(2:end)) ;
 %    U(n,:) = avg(y).*(ly - avg(y)) ;
...
for k = 1:nt
   ...
   % Compute the error between the steady state and the solution at time t.
   
  
    
    % Let the code run once so that we have the Ue of the previour time
    % step, then begin checking the relative error
    
    if k > 1
        % Compute the error
        Error = norm((Ue - Uinter)/norm(Ue)) ;
     
        if Error < tol
            disp(['Time taken to reach steady state = ' num2str(k*dt) ])
            ...
            % Plot the U component of the velocity field
            figure(1)
            hold on
            colorbar
            contourf(x(2:end-1),avg(y),Usteady',15)
            %title(['Steady state of U at t = ' num2str(k*dt) ' for Re = ' num2str(Re)])
            title('Exact steady state for U')
            hold off  
            
            % Plot the V component of the velocitu field
            figure(2)
            hold on
            caxis([min(min(Ue)),max(max(Ue))])
            colorbar
            contourf(avg(x),y(2:end-1),Vsteady',0)
            %title(['Steady state of V at t = ' num2str(k*dt) ' for Re = ' num2str(Re)]) 
            title('Exact steady state for V')
            hold off 
