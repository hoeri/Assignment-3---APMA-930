...
for k = 1:1
   ...
   %Correct P with chorins method
   %P = P - diff([uW;Unew;uE])/hx+diff([vS' Vnew vN']')'/hy ;
   ...
   figure(1)
   plot(avg(y),DP,'-','Color',[Col/6, 0 ,0],'LineWidth',2, 'DisplayName', ['dt = ', num2str(dt)])
   legend('-DynamicLegend', 'Location', 'north')
   title('Corrected Pressure Gradient')
   xlabel('y range')
   ylabel('Pressure Gradient')
   hold on
   
   %,'Color',[Col/6, 0 ,0],'LineWidth',2, ...
   
   % Store the difference between exact and computed gradients
   
   InfNorm = [InfNorm norm(DP - DPexact,inf) ];
...
plot(avg(y), DPexact,'ro','DisplayName', ['Exact Gradient'])
%legend('Largest','2','3','4','5','Smallest','Exact')
hold off
