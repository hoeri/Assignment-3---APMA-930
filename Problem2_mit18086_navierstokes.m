...
for k = 1:nt*1
   ...
   if k==1|floor(nsteps*k/nt)>floor(nsteps*(k-1)/nt)
      ...
      % Compute the vorticity from u, and reshape it
      Vort = (diff(U')'/hy - diff(V)/hx) ;
      Vort(1:end,1:end) = reshape(Vort,nx-1,ny-1);
      XX = x(1,2:end-1) ;
      YY = y(1,2:end-1) ;
      
      clf, contourf(avg(x),avg(y),P',20,'w-'), 
      hold on
      %Plot the stream function Q
      %contour(x,y,Q',20,'k-');
      
      % Plot the vorticity
      contour(XX,YY,Vort',20,'k-','LineWidth',2);
      ...
      title(sprintf('Vorticity with, Re = %0.1g at  t = %0.2g',Re,k*dt))
      drawnow
   end
end
fprintf('\n')
